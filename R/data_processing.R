...
blastp.results <- as.data.frame(
  pivot_wider(read.table("results/blastpResults.tsv")[c("V1", "V2", "V12")],
              names_from = "V1", values_from = "V12")
  )
...
blastp.results <- blastp.results[
  sort(row.names(blastp.results)),
  sort(colnames(blastp.results))
  ]
